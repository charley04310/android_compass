...
fun ReplyDockedSearchBar(
    ...
    onSearchItemSelected: (PlaceToCompass) -> Unit,
    modifier: Modifier = Modifier
) {
    var query by remember { mutableStateOf("") }
    var active by remember { mutableStateOf(false) }
    val searchResults = remember { mutableStateListOf<PlaceToCompass>() }

    LaunchedEffect(query) {
        searchResults.clear()
        if (query.isNotEmpty()) {
           // searchResults.addAll(
             //   placeToCompasses.filter {
        ...
        if (searchResults.isNotEmpty()) {
            LazyColumn(
                ...
                items(items = searchResults, key = { it.id }) { email ->
                    ListItem(
                        headlineContent = { Text(email.country) },
                        supportingContent = { Text(email.name) },
                        leadingContent = {
                            CompassProfileImage(
                                drawableResource = email.avatar,
                                description = stringResource(id = R.string.profile),
                                modifier = Modifier
                                    .size(32.dp)
                            )
